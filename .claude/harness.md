...
`[y/n/s]` gates. Phases 1–5 are deterministic (no LLM, no API key); Phases 6–9 are hybrid
...
| 08 | [diagram](agents/08-diagram.md) | Deterministic | 8a | `phases/p08_diagram/diagram_builder.py` | `diagram/*.mmd` · `diagrams_artifact.json` |
| 09 | [brd](agents/09-brd.md) | LLM + Python | 8 | `phases/p09_brd/brd_builder.py` + `phases/p09_brd/brd_agent.md` | `final_report/brd.md` · `gaps_register.*` |
| 10 | [judge](agents/10-judge.md) | LLM + Python | 9 | `phases/p10_judge/brd_judge.py` + `phases/p10_judge/brd_judge_agent.md` | `final_report/brd_judge.md` / `.json` |

*Diagram (8a) and BRD (8) are both part of pipeline Phase 8; the diagram runs first so the
BRD can embed the `*.mmd` files.*
...
COBOL src
  ...
  → 06 logic_artifact.json (+ program_logic/*)              [LLM]
  → 07 rules_artifact.json (+ classified_conditions.json)   [LLM]
  → 08 *.mmd + diagrams_artifact.json                        [deterministic]
  → 09 brd.md + gaps_register.*                              [LLM]
  → 10 brd_judge.md/.json  →  PASS | REVISE → back to 09     [LLM]
```
...
# Phases 1–5: pure Python, instant, no key.  Phases 6–9: LLM (default answer = skip).
```
